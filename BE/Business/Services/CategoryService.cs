...
public class CategoryService : BaseService<CategoryResource, CreateCategoryResource, UpdateCategoryResource, Category>, ICategoryService
{
    ...
    #region Method
    public override async Task<BaseResult<CategoryResource>> InsertAsync(CreateCategoryResource createCategoryResource)
    {
        try
        {
            ...
            using (hasValue.GetEnumerator())
                if (hasValue.GetEnumerator().MoveNext())
                    return new BaseResult<CategoryResource>(ResponseMessage.Values["Category_Existent"]);
            ...
            return new BaseResult<CategoryResource>(Mapper.Map<Category, CategoryResource>(tempCategory));
        }
        catch (Exception ex)
        {
    ...
    public override async Task<BaseResult<CategoryResource>> UpdateAsync(int id, UpdateCategoryResource updateCategoryResource)
    {
        try
        {
            ...
            if (tempCategory is null)
                return new BaseResult<CategoryResource>(ResponseMessage.Values["Category_NoData"]);
    ...
    public async Task<PaginationResult<IEnumerable<CategoryResource>>> GetPaginationAsync(QueryResource pagination, FilterCategoryResource filterResource)
    {
        ...
        var resource = new PaginationResult<IEnumerable<CategoryResource>>(tempResource);
