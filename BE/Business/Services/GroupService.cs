...
public class GroupService : BaseService<GroupResource, CreateGroupResource, UpdateGroupResource, Group>, IGroupService
{
    ...
    #region Method
    public async Task<BaseResult<GroupResource>> AddGroupToAccountAsync(int accountId, int groupId)
    {
        try
        {
            ...
            if (tempAccount is null)
                return new BaseResult<GroupResource>(ResponseMessage.Values["Account_NoData"]);

            // Validate group-Id is existent?
            var tempGroup = await _groupRepository.GetByIdAsync(groupId);
            if (tempGroup is null)
                return new BaseResult<GroupResource>(ResponseMessage.Values["Group_NoData"]);


            tempAccount.Groups = new() { tempGroup };
            await UnitOfWork.CompleteAsync();

            return new BaseResult<GroupResource>(Mapper.Map<Group, GroupResource>(tempGroup));
        }
        catch (Exception ex)
        {
    ...
    public async Task<BaseResult<IEnumerable<PersonResource>>> GetListPersonByGroupIdAsync(int groupId)
    {
        ...
        return new BaseResult<IEnumerable<PersonResource>>(personResource);
    }

    public async Task<BaseResult<GroupResource>> RemoveGroupFromAccountAsync(int accountId, int groupId)
    {
        try
        {
            // Validate account-Id is existent?
    ...
    public override async Task<BaseResult<GroupResource>> InsertAsync(CreateGroupResource createGroupResource)
    {
        try
        {
            ...
            return new BaseResult<GroupResource>(tempResource);
        }
        catch (Exception ex)
        {
            throw new MessageResultException(ResponseMessage.Values["Group_Saving_Error"], ex);
    ...
    public override async Task<BaseResult<GroupResource>> UpdateAsync(int id, UpdateGroupResource updateGroupResource)
    {
        try
        {
    ...
    public async Task<PaginationResult<IEnumerable<GroupResource>>> GetPaginationAsync(QueryResource pagination, FilterGroupResource filterResource)
    {
        var totalTechnology = await _technologyService.GetAllAsync();
        ...
        var resource = new PaginationResult<IEnumerable<GroupResource>>(tempResource);
