...
public class GroupController : DongNguyenController<GroupResource, CreateGroupResource, UpdateGroupResource, Group>
{
    ...
    [ProducesResponseType(typeof(BaseResult<IEnumerable<GroupResource>>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(BaseResult<IEnumerable<GroupResource>>), StatusCodes.Status204NoContent)]
    [ProducesResponseType(typeof(BaseResult<IEnumerable<GroupResource>>), StatusCodes.Status400BadRequest)]
    ...
    public async Task<IActionResult> FindAsync([FromQuery] string filterName)
    {
        ...
        return Ok(new BaseResult<IEnumerable<GroupResource>>(Mapper.Map<IEnumerable<Group>, IEnumerable<GroupResource>>(result)));
    }

    [HttpGet("list-person-view/{id:int}")]
    [Authorize(Roles = $"{Role.Admin}, {Role.EditorQTNS}, {Role.EditorQTDA}, {Role.Viewer}")]
    [ProducesResponseType(typeof(BaseResult<GroupResource>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(BaseResult<GroupResource>), StatusCodes.Status204NoContent)]
    [ProducesResponseType(typeof(BaseResult<GroupResource>), StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> GetListPersonForViewAsync(int id)
    {
        ...
        return Ok(new BaseResult<IEnumerable<PersonResourceView>>(mapObj));
    }
    ...
    public async Task<IActionResult> GetListPersonForEditAsync(int id)
    {
        ...
        return Ok(new BaseResult<IEnumerable<PersonResource>>(result.Resource));
    }
    ...
    [HttpPost]
    [Authorize(Roles = $"{Role.Admin}, {Role.EditorQTNS}, {Role.EditorQTDA}")]
    [ProducesResponseType(typeof(BaseResult<GroupResource>), StatusCodes.Status201Created)]
    [ProducesResponseType(typeof(BaseResult<GroupResource>), StatusCodes.Status400BadRequest)]
